...
app.delete('/events/:id', async (req, res) => {
    ...
    try {
        ...
        if (!result) return res.status(404).json({ error: 'Подію не знайдено або не ваша' });
...
app.post('/participants', async (req, res) => {
    try {
        ...
        if (existing) return res.status(409).json({ error: 'Ви вже зареєстровані на цю подію' });
            ...
            return {
                ...
                pageInfo: {
                    ...
                    endCursor: nodes.length > 0
                        ? Buffer.from(nodes[nodes.length - 1]._id.toString()).toString('base64')
                        : null
                },
                totalCount
            };
